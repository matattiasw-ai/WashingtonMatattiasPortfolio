...
def main(page: ft.Page):
    page.title = "Washington Matattias - SiteSpy Portfolio"
    ...
    hero = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.Text("Washington Matattias", size=34, weight=ft.FontWeight.W_900, color=TEXT),
            ft.Text("GitHub: matattiasw-ai | Email: matattiasw@gmail.com", color=MUTED, selectable=True),
            ft.Text("Integration review, package scripts, and final coordination docs.", size=16, color=TEXT),
            ft.Text("GitHub profile: https://github.com/matattiasw-ai", color=GOLD, selectable=True),
        ], spacing=13),
    )
        ...
        ft.Container(
            ...
            content=ft.ResponsiveRow([
                ...
                ft.Container(tile('Contribution Area', [
                    ft.Text("Integration review, package scripts, and final coordination docs.", color=TEXT),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Files / Validation Area', [
                    ft.Text('- README.md', color=MUTED, size=13),
                    ft.Text('- package.json', color=MUTED, size=13),
                    ft.Text('- project-management/work-division.md', color=MUTED, size=13),
                    ft.Text('- project-management/final-integration-plan.md', color=MUTED, size=13),
                ], ACCENT), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Local Git Setup', [
                    ft.Text("git config user.name \"Washington Matattias\"\ngit config user.email \"matattiasw@gmail.com\"\ngit config --get user.name\ngit config --get user.email", color=TEXT, selectable=True),
                ]), col={'xs': 12, 'md': 6}),
                ft.Container(tile('Clone, Commit, Push', [
                    ft.Text("git clone \"Replace with your own fork URL after creating the fork.\"\ncd \"Replace with your cloned repository folder\"\ngit status\ngit checkout -b portfolio/matattias-w-sitespy-portfolio\ngit add README.md index.html main.py\ngit commit -m \"Update Washington Matattias SiteSpy portfolio\"\ngit push -u origin portfolio/matattias-w-sitespy-portfolio", color=TEXT, selectable=True),
